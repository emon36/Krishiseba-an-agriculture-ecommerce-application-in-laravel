...
<div class="content-wrapper">
          <div class="row">
            ...
            <div class="card-body">
               ...
               <form action="{{ route('admin.category.update', $category->id)}}" method="post" enctype="multipart/from-data">
                {{ csrf_field() }}
                @include('admin.partials.messeage')
  <div class="form-group">
    <label for="name">Name</label>
    <input type="text" class="form-control" name="name" id="name" aria-describedby="Titlehelp" value="{{$category->name}}" placeholder="Enter product category name">
    <small id="Titlehelp" class="form-text text-muted">Input a  valid product category</small>
  </div>
  <div class="form-group">
    <label for="Inputdescription">Description</label>
    <textarea name="description" rows="7" cols="70" class="form-control">{{$category->description}}</textarea>
  </div>
